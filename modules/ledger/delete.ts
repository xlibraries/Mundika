...
export async function deleteLedgerEntry(
  ...
  if (row.ref_bill_id) {
    throw new Error(
      "This entry is linked to a bill. Delete the bill from Dashboard → Transactions (Billing) instead."
    );
  }
  if (row.ref_purchase_id) {
    throw new Error(
      "This entry is linked to a purchase. Delete it from Dashboard → Transactions (Purchase) instead."
    );
  }
